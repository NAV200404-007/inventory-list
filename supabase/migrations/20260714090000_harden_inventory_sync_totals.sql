-- Keep inventory totals and active item IDs in lock-step. This protects the
-- app from stale active assets left over after totals are reduced.
...
declare
  ...
  item_id text;
  item_total integer;
  touched_item_ids text[] := array[]::text[];
begin
  ...
  loop
    ...
    touched_item_ids := array_append(touched_item_ids, item_id);

    select coalesce(array_agg(code), array[]::text[])
    into desired_codes
    from (
      select upper(trim(value #>> '{}')) as code
